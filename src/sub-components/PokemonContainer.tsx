...
const PokemonContainer: React.FC<PokemonContainerProps> = ({ pokemon }) => {
    ...
    return (<div
        ...
        onClick={() => pokemon.id < 10000 && navigate(`/pokemon/${pokemon.name}`)}
        className={`${hovered ? `${hoveredBgColor} bg-opacity-35` : 'bg-white'} w-fit h-80 rounded-3xl space-y-3 flex flex-col justify-end items-center p-8 shadow-lg scale-90 xl:mt-16 mt-8 transition-all hover:-translate-y-4 ${pokemon.id < 10000 && 'hover:cursor-pointer'}`}>
